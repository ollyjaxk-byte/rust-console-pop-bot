  ...
  try {
    ...
    if (i.isModalSubmit()) {
      ...
      if (i.customId === 'server_modal') {
        ...
        return i.reply({ content: '✅ Server details saved. Add the private RCON password to finish.', components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('rco_add').setLabel('Add RCON password').setEmoji('🔐').setStyle(ButtonStyle.Primary))], ephemeral: true });
      }
      if (i.customId === 'credential_modal') { const p = serverProfiles.get(i.guild.id); if (!p) return i.reply({ content: 'Configure server details first.', ephemeral: true }); p.rcoPassword = i.fields.getTextInputValue('rco_password').trim(); serverProfiles.set(i.guild.id, p); return i.reply({ content: '✅ RCON setup complete. Password stored privately in memory and never displayed.', ephemeral: true }); }
    }
