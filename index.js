...
const serverModal=()=>new ModalBuilder().setCustomId('server_modal').setTitle('🛠️ Rust server details').addComponents(
 ...
 new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('rcon_port').setLabel('WebRCON port').setPlaceholder('WebRCON port from your host').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(10)),
...
const credentialModal=()=>new ModalBuilder().setCustomId('credential_modal').setTitle('🔐 WebRCON credentials').addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('rco_password').setLabel('WebRCON password').setPlaceholder('Entered privately — never posted').setStyle(TextInputStyle.Short).setRequired(true).setMaxLength(200)));
const profileCard=(p)=>brand('SERVER PROFILE // LIVE CONNECTION').setDescription('The bot uses these details for live WebRCON checks. Secrets stay hidden.').addFields({name:'🧱 Server',value:p.name,inline:true},{name:'📍 WebRCON endpoint',value:p.ip+':'+(p.rconPort||'—'),inline:true},{name:'🌎 Region',value:p.region,inline:true},{name:'📝 Description',value:p.description||'No description'}, {name:'🔐 Credentials',value:'Configured • hidden',inline:true});
