...
client.on('interactionCreate', async (i) => {
  try {
    ...
    if (i.isButton()) {
      if (i.customId === 'server_add') { if (i.guild?.ownerId !== i.user.id) return i.reply({ content: '🔒 Owner only.', ephemeral: true }); try { return await i.showModal(serverModal()); } catch (error) { if (error?.code === 10062) return; throw error; } }
      if (i.customId === 'rco_add') { if (i.guild?.ownerId !== i.user.id) return i.reply({ content: '🔒 Owner only.', ephemeral: true }); try { return await i.showModal(credentialModal()); } catch (error) { if (error?.code === 10062) return; throw error; } }
