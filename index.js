...
async function fetchRconSnapshot(guildId){const p=serverProfiles.get(guildId);if(!p)throw Error('Run /pop setup and configure the Rust server first');if(!p.rconPassword)throw Error('RCON password is missing. Open /pop setup and click Add RCON password.');const [infoRaw,playersRaw,statusRaw,usersRaw]=await Promise.all([rconRequest(p,'server.info'),rconRequest(p,'playerlist'),rconRequest(p,'status'),rconRequest(p,'users')]);const info=parseJsonOrText(infoRaw);const players=parseJsonOrText(playersRaw);const rows=parsePlayers(players?.players??players?.Players??players?.data?.players??playersRaw);const current=Number(info?.players??info?.currentPlayers??info?.online??info?.playerCount??rows.length);const d={serverName:p.name,current:Number.isFinite(current)?current:rows.length,max:info?.maxPlayers??info?.maxplayers??info?.capacity??null,rows,info,status:statusRaw,users:usersRaw,queue:info?.queue??info?.joining??null,map:info?.map??info?.level??null};rconCache.set(guildId,{data:d,at:Date.now(),error:null});return d;}
