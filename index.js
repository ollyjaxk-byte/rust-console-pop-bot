  ...
  try {
    if (i.isChatInputCommand()) {
      ...
      if (sub === 'setup') {
        ...
        return i.reply({ embeds: [brand('POP SETUP // SERVER CONTROL').setDescription('Configure the Rust Console server directly inside Discord. The RCON password is accepted privately and never displayed.').addFields({ name: 'Automatic updates', value: config.get(i.guild.id) || 'off' }, { name: 'Saved profile', value: serverProfiles.has(i.guild.id) ? '✅ Configured' : '⚪ Not configured' })], components: [setupButtons(), updateSetup()], ephemeral: true });
      }
      if (sub === 'test') {
